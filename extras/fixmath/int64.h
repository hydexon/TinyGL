...
#define INIT_INT64T(x, h, l) _int64_t x; x.hi = h; x.lo = l;

static inline _int64_t int64_const(int32_t hi, uint32_t lo) {  _int64_t c;  c.hi = hi; c.lo = lo; return c; }
static inline _int64_t int64_from_int32(int32_t x) { _int64_t i; i.hi = (x < 0 ? -1 : 0); i.lo = x; return i;}
...
static inline _int64_t int64_mul_i32_i32(int32_t x, int32_t y) {
	 ...
	 int16_t hi[2]; hi[0] = (x >> 16); hi[1] = (y >> 16);
	uint16_t lo[2]; lo[0] = (x & 0xFFFF); hi[1] = (y & 0xFFFF);

	 int32_t r_hi = hi[0] * hi[1];
	 int32_t r_md = (hi[0] * lo[1]) + (hi[1] * lo[0]);
	uint32_t r_lo = lo[0] * lo[1];
	
	INIT_INT64T(r_hilo64, r_hi, r_lo);
...
static inline _int64_t int64_mul_i64_i32(_int64_t x, int32_t y) {
	...
	uint32_t _x[4]; _x[0]=(x.lo & 0xFFFF); _x[1] = (x.lo >> 16); _x[2] = (x.hi & 0xFFFF); _x[3] = (x.hi >> 16);
	uint32_t _y[2]; _y[0]=(ypos & 0xFFFF); _y[1] = (ypos >> 16);
